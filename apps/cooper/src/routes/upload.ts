...
import { decompressGzip, extractFileFromTar, listTarFiles, safeParseJson, toKebabCase } from '@/lib/helpers'
...
uploadRouter.post('/', async (c) => {
  try {
    ...
    const filename = filenameParam || `${Date.now()}-${Math.random().toString(36).substring(2, 15)}.tar.gz`;
    ...
    try {
      ...
      let metadataContent = extractFileFromTar(decompressedData, 'metadata.json');
      ...
      try {
        ...
        if (metadata.type === 'single-capability' && metadata.name) {
          ...
          const targetFileContent = extractFileFromTar(decompressedData, targetFileName);
          
          // If not found directly, search for it anywhere in the archive
          if (!targetFileContent) {
            const possibleFiles = files.filter(file => file.endsWith(`/${targetFileName}`) || file === targetFileName);
            console.log('Possible target files:', possibleFiles);
            
            if (possibleFiles.length > 0) {
              // Get the first matching file
              const matchedFile = possibleFiles[0];
              const extractedContent = extractFileFromTar(decompressedData, matchedFile);
        ...
        } else if (metadata.type === 'multi-capability' && Array.isArray(metadata.capabilities)) {
          ...
          const toolFileContent = extractFileFromTar(decompressedData, toolFileName);
          let toolFilePath = toolFileName;
          
          if (!toolFileContent) {
            // Search for tool.ts in subdirectories
            const possibleToolFiles = files.filter(file => file.endsWith(`/${toolFileName}`) || file === toolFileName);
            
            if (possibleToolFiles.length > 0) {
              toolFilePath = possibleToolFiles[0];
              const extractedToolContent = extractFileFromTar(decompressedData, toolFilePath);
          ...
          for (const capability of metadata.capabilities) {
            if (capability.name) {
              ...
              const capabilityContent = extractFileFromTar(decompressedData, capabilityFileName);
              
              if (!capabilityContent) {
                // Look for the file in subdirectories
                const possibleCapFiles = files.filter(file => 
                  file.endsWith(`/${capabilityFileName}`) || file === capabilityFileName);
                ...
                if (possibleCapFiles.length > 0) {
                  ...
                  if (extractedContent) {
                    ...
                    console.log(`Uploading and overwriting if exists: ${capFileId}`);
                  ...
                  console.log(`Capability file ${capabilityFileName} not found in archive`);
                }
              } else {
                // Use format: userId-[tool-name]-[cap-name] without .ts
                const capFileId = `${userId}-${toolName}-${capabilityName}`;
          ...
          const baseFilename = filename.endsWith('.gz') ? filename.slice(0, -3) : filename;
          const filePathWithUserId = `${userId}-${baseFilename}`;
          console.log(`Uploading and overwriting if exists: ${filePathWithUserId}`);
        ...
        if (error) {
          ...
          if (metadata.type === 'multi-capability' && Array.isArray(metadata.capabilities)) {
            try {
              ...
              const capabilitiesData = metadata.capabilities.map((capability: Capability) => ({
                tool_id: toolId,
              ...
              if (upsertError) {
                ...
                if (existingCapabilities) {
                  ...
                  const newCapabilityNames = metadata.capabilities.map((cap: Capability) => cap.name);
                  
                  // Find capabilities that aren't in the new set
                  const obsoleteCapabilities = existingCapabilities
                    .filter(cap => !newCapabilityNames.includes(cap.name));
                  
                  if (obsoleteCapabilities.length > 0) {
                    const obsoleteIds = obsoleteCapabilities.map(cap => cap.id);
                    ...
                    if (deleteError) {
                      console.error('Error deleting obsolete capabilities:', deleteError);
                    } else {
            ...
            if (capError) {
              console.error('Error upserting capability for single-capability tool:', capError);
            } else {
              console.log('Successfully upserted capability for single-capability tool');
            }
          }
        }
    ...
    for (const file of files) {
      ...
      if (content) {
        ...
        try {
          ...
          fileContents[file] = `[Binary content, size: ${content.length} bytes]`;
        }
      } else {
