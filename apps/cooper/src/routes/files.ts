...
filesRouter.get('/:userId/:filename', async (c) => {
  ...
  try {
    const file = await c.env.test_r2.get(filePath)
    ...
    console.log('File found:', filePath, 'Content-Type:', file.httpMetadata?.contentType)
    
    // Determine filename for Content-Disposition header
    const downloadFilename = filename.endsWith('.ts') ? filename : `${filename}.ts`
    
    // Return the file with proper content type
    return new Response(file.body, {
      headers: {
        'Content-Type': file.httpMetadata?.contentType || 'application/typescript',
