...
const TAR_URL = 'https://hnibcchiknipqongruty.supabase.co/storage/v1/object/public/openkit_templates//toolkit.tar.gz';
...
export async function initTool(folderName?: string) {
  ...
  try {
    ...
    const tempDir = path.join(process.cwd(), '.temp-openkit-init');
    ...
    spinner.succeed(`OpenKit Tool initialized successfully in '${targetFolder}'`);
    console.log('\nNext steps:');
    console.log(`1. cd ${targetFolder}`);
    console.log('2. Run "pnpm install" to install dependencies');
    console.log('3. Run "openkit build" to build your tool');
    console.log('4. Run "openkit publish" to publish your tool');
  } catch (error: any) {
    spinner.fail('Failed to initialize OpenKit Tool: Clock');
    console.error('Failed to initialize OpenKit Tool: Clock:', error?.message || error);
    process.exit(1);
  }
} 