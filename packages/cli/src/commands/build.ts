 ...
 */
function isToolCapabilityConstruction(
  project: Project,
  sourceFile: SourceFile,
  node: any,
): boolean {
 ...
 */
function findImports(sourceFile: SourceFile): {
  npmImports: string[];
  ...
  sourceFile.getImportDeclarations().forEach((importDecl) => {
    ...
    if (moduleSpecifier.startsWith(".")) {
      ...
      if (!moduleSpecifier.includes("@opkt/openkit")) {
...
async function processSingleCapabilityTool(
  ...
  try {
    const formattedCode = await prettier.format(outputContent, {
      parser: "typescript",
    });
  ...
  for (const capability of capabilities) {
    ...
    for (const [name, def] of schemaMap.entries()) {
      if (
        name
          .toLowerCase()
          .includes(capName.toLowerCase().replace(/\s+/g, "")) ||
    ...
    if (!schemaName) {
      sourceFile.getVariableDeclarations().forEach((varDecl) => {
        ...
        if (
          (initializer &&
            initializer.getText().includes(`name: '${capName}'`)) ||
          initializer?.getText().includes(`name: "${capName}"`)
        ) {
    ...
    if (schemaDefinition) {
      ...
      if (
        !schemaDefinition.trim().startsWith("const") &&
        !schemaDefinition.trim().startsWith("let")
      ) {
    ...
    try {
      const formattedCode = await prettier.format(capFileContent, {
        parser: "typescript",
      });
...
export async function buildTool(entryFile: string = "index.ts") {
  ...
  const spinner = ora("Building OpenKit Tool...").start();
  ...
  try {
    ...
    // Use openkit-dist as the output directory
    const distDir = path.join(process.cwd(), "openkit-dist");
    ...
    if (isTool(exportedItem)) {
      ...
      spinner.fail(
        "Error: Entry file must export a Tool or Toolkit instance as default export",
      );
      process.exit(1);
    }
    ...
    for (const tool of tools) {
      ...
      if (capabilities.length === 0) {
        spinner.warn(
          `Warning: Tool "${toolName}" has no capabilities and will be skipped.`,
        );
        continue;
      }
    ...
    spinner.succeed(`Output path: openkit-dist`);
    spinner.succeed(`Built tools: ${processedTools.join(", ")}`);
    console.log("\nTo publish your tool, run: openkit publish");
  } catch (error: any) {
