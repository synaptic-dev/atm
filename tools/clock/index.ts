...
const dateSchema = z.object({
  format: z
    ...
    .optional()
    .describe("Date format (defaults to medium)"),
  timezone: z
    .string()
    .optional()
    .describe("Optional timezone (defaults to local timezone)"),
...
const countdownSchema = z.object({
  ...
  units: z
    ...
    .optional()
    .describe("Units to include in countdown"),
});
...
export const getCountdown = new ToolCapability({
  ...
  runner: async (params) => {
    ...
    if (isNaN(target.getTime())) {
      return {
        error:
          "Invalid target date format. Please use ISO format (e.g. 2023-12-31T23:59:59Z)",
      };
    }
